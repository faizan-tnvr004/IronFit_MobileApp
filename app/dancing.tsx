...
export default function DancingScreen() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const activityLogs = data.filter(a => a.type === 'Dancing');
  ...
  if (logs.length > 0) {
    ...
    logs.forEach(log => {
      ...
      if (diffDays >= 0 && diffDays < 7) {
        chartData[6 - diffDays] += log.caloriesBurned;
      }
    });
  }
  ...
  const weekData = {
    labels,
    datasets: [{ data: chartData.some(d => d > 0) ? chartData : [1, 1, 1, 1, 1, 1, 1] }],
  };
  ...
  return (
    <SafeAreaView style={s.safe}>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <View style={s.chartCard}>
              <Text style={s.chartTitle}>Energy Burned (kcal)</Text>
              <BarChart
                data={weekData}
                width={W - 64}
                height={180}
      ...
      <View style={s.footer}>
        <TouchableOpacity style={[s.startBtn, { backgroundColor: '#A855F7' }]} onPress={() => setShowModal(true)}>
        ...
        <View style={s.modalOverlay}>
          <View style={s.modalCard}>
            ...
            <View style={s.modalActions}>
              ...
              <TouchableOpacity style={[s.modalSave, { backgroundColor: '#A855F7' }]} onPress={handleLogActivity} disabled={isSubmitting}>
