    ...
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.container}>
        ...
        <View style={s.statsGrid}>
          {[
            { label: 'Session Time', value: '60',  unit: 'min' },
            { label: 'Calories',     value: '420', unit: 'kcal' },
            { label: 'Heart Rate',   value: '125', unit: 'BPM avg' },
          ].map((stat) => (
            <View key={stat.label} style={[s.statCard, stat.label === 'Heart Rate' && { width: '100%' }]}>
        ...
        <View style={s.chartCard}>
          <Text style={s.chartTitle}>Energy Burned</Text>
          <BarChart
...
/>
      ...
      <View style={s.footer}>
        <TouchableOpacity style={[s.startBtn, { backgroundColor: '#A855F7' }]} onPress={() => Alert.alert('Coming Soon', 'This functionality is not yet defined.')}>
          <FontAwesome name="play" size={16} color="#fff" style={{ marginRight: 10 }} />
          <Text style={s.startBtnText}>Start Dancing</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
