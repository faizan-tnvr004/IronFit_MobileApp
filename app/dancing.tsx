...
export default function DancingScreen() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setLogs(data.filter(a => a.type === 'Dancing'));
    } catch (err) {
      console.error(err);
    } finally {
      setIsLoading(false);
  ...
  const handleLogActivity = async () => {
    ...
    try {
      ...
      const calories = calculateCalories('Dancing', durNum, userProfile.weightKg);
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.container}>
        ...
        <Text style={s.title}>Dancing</Text>
        {isLoading ? (
      ...
      <View style={s.footer}>
        <TouchableOpacity style={[s.startBtn, { backgroundColor: '#EC4899' }]} onPress={() => { setPickerMode('date'); setShowDatePicker(true); setShowModal(true); }}>
      ...
      <Modal visible={showModal} transparent animationType="slide">
        <View style={s.modalOverlay}>
          <View style={s.modalCard}>
            <Text style={s.modalTitle}>Log Dancing Session</Text>
            ...
            <View style={s.modalActions}>
              ...
              <TouchableOpacity style={[s.modalSave, { backgroundColor: '#EC4899' }]} onPress={handleLogActivity} disabled={isSubmitting}><Text style={s.modalSaveText}>{isSubmitting ? 'Saving...' : 'Save'}</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
