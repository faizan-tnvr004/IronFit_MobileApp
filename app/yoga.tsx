...
export default function YogaScreen() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setLogs(data.filter(a => a.type === 'Yoga'));
    } catch (err) {
      console.error(err);
    } finally {
      setIsLoading(false);
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.footer}>
        <TouchableOpacity style={[s.startBtn, { backgroundColor: '#A855F7' }]} onPress={() => { setPickerMode('date'); setShowDatePicker(true); setShowModal(true); }}>
        ...
        <View style={s.modalOverlay}>
          <View style={s.modalCard}>
            ...
            <View style={s.modalActions}>
              ...
              <TouchableOpacity style={[s.modalSave, { backgroundColor: '#A855F7' }]} onPress={handleLogActivity} disabled={isSubmitting}><Text style={s.modalSaveText}>{isSubmitting ? 'Saving...' : 'Save'}</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
